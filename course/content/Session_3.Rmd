...
As we've seen, code blocks wrapped within triple back ticks (\```{r} ... ```) will be executed and included inline with the surrounding text. These can be inserted at the position of the cursor with `⌘+⌥+I` (Mac) or `CTRL+Alt+I` (Windows).

The surrounding text can be annotated with a variety of formatting marks - see the excellent [cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf) that summarises the common ones - that are interpreted by `knitr`.
...
Let's write a simple function to multiply two numbers.
...
When running, functions - both in-built and user-defined - operate within an individual, separated, *scope*. Within a function's scope, or 'sub-enviornment':

- variables from the parental scope are available within the function as **read-only** copies
...
`apply()`, and other functions that list `...` in their function signature, allow for *pass through* of extra arguments that aren't relevant to them.
...
In order to make use of them, a library's functions must first be loaded into your `R` environment. As seen for `install()` from `BiocManager` and for `install_github()` from `devtools`, the package is loaded using the `library()` function.
...
Once a package has been loaded, we can use the functions directly, or can refer to them by referencing the package that provides them as well.
...
It can help to improve the clarity of your code by referring to functions explicitly. Sometimes, however, it's necessary - for example when two packages supply a function of the same name.
...
This output includes links to packages *vignettes* - HTML pages or PDFs that walk through how to use a package, often following a dummy workflow with an exemplar dataset.

If you want to get to grips with the basic functionality of a package, it's vignette is the place to start!
...
Some (gentle) streamlining our data as this point is a good way of decreasing processing time and requirements downstream.
...
- perform the above filter, but using a different cutoff (e.g. `>=10`)
...
As we have a read-made matrix, we use the `DESeqDataSetFromMatrix()` function to set up a `DESeqDataSet` object, but there are several equivalents that allow integration of data from different sources.
...
We haven't seen anything quite like this so far. A `DESeqDataSet` object is an object *class* itself, of which `dds` is an example. 
...
Our `dds` object contains a single assay that stores our raw expression data: `counts`. We can access this using the generic `assay()` function ...

```{r}
head(assay(dds, "counts"), 3)
```

... or using the special `counts()` function built into the `DESeqDataSet` object.
...
```{r}
res = results(dds, contrast=c("Species", "Adelie", "Gentoo"))
```




# Generating results with Bayesian Shrinkage of Log2 Fold Changes

* The authors of DESeq2 recommend applying a more sophisticated approach to reporting log2FC than the original `results()` function
* The function `lfcshrink()` generates a results table using additional statistical methods
* The `apeglm` algorithm is good at squishing low-intensity, noisy Log2FC values but not the less noisy ones
* This could be useful when ranking genes by Log2FC for GSEA etc
