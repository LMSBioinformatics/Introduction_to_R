...
The data in this course is carefully curated. Real data is messy. It's worth going over some of the arguments to `read.delim()` and `read.csv()` that might help you import your own data:
...
```{r, eval=FALSE}
data = read.delim("https://www.example.com/myfile.tsv")
```
...
```{r, eval=FALSE}
x = data.frame(a=c(1, 2, 3), b=("a", "b", "c"))
...
Each plot created before `dev.off()` is called will be created on a new page in the resulting document.
...
`R` objects can be saved to allow important steps in a workflow to be preserved or to save the results of a lengthy computation. The `save()` function can be used to store specific objects ...
...
Overwriting a variable with the special `NULL` constant ...
...
... will remove it from use but doesn't actually free its memory. We can encourage `R` to do perform this second step by manually calling the `gc()` (garbage collection) function.
...
Complementing the data frame, the `matrix` is the second 2D tabular structure in base `R`. In comparison to the data frame, all data within a matrix must be of the same underlying class: `numeric`, `character`, or `logical`.
...
Sometimes it's useful to force control the coercion yourself. Each basic type has a matched `as.` function that will convert an input appropriately.
...
Best practice, therefore, if not to find yourself in this situtation in the first place!
...
```{r}
tapply(x, c(rep(1, 3), rep(2, 3), rep(3, 3)), sqrt)
```
