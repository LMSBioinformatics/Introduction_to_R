...
An object can be deleted using the `rm()` (or `remove()`) functions ...
...
... which will remove it from use but doesn't actually free its memory. We can encourage `R` to do perform this second step by manually calling the `gc()` (garbage collection) function.
...
Best practice, therefore, is not to find yourself in this situtation in the first place!
