...
We can highjack this system to 'comment out' code sections if we don't want them to run.
...
mean(x=one_to_tex)  # ... this. The names of positional arguments are implicit
```
...
- `Numeric` values (stored identically whether they have a floating point or not) store a number
...
Here, `R` uses the `NA` (not available) logical constant to signify that a value is missing.
...
Again, `R` uses the `NA` (not available) logical constant to signify that a value is missing.
...
- Which of penguins are too young to breed?
...
- `view()`, which in `RStudio` will open a separate tab to display the contents

- `str()` to view its structure and the basic types of its component vectors

- `table()` and `summmary()` to view compact representations of columns
...
We can *sanitise* an entire data frame by using the `na.omit()` function to drop `NA`-containing rows ...

```{r, eval=FALSE}
penguins = na.omit(penguins)
```

... but, unless we want to discard data, it's often better to just drop `NA`s from vectors during calculations.

```{r}
range(na.omit(penguins$Height))
```
...
We will cover plotting in more detail using the `ggplot2` library in **Session 4**. 'Base' `R` can still do some basic plotting, though, and has four types of graph built in:
...
Columns defined as factors can be used to modify the aesthetics of the plot. For example, we can edit the colour and shape of points based on `Sex`. It makes sense to then also use the `legend()` function to show what we've done.
