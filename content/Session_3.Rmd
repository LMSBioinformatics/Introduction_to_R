...
[Markdown cheatsheet](../content/assets/rmarkdown_cheatsheet.pdf)
...
- `-`, `*`, and `+` bullet (indent for sub-bullets)
...
`DESeq2` provides methods to test for differential gene expression using negative binomial regression and allows testing against specified fold change thresholds.

Fundamentally, `DESeq2` requires two inputs:

- Sample metadata. A data frame detailing the various biological and technical factors relating to the experiment (Age, Sex, Treatment, Replicate, etc)

- A matrix of un-normalized count data. Count values pre-normalized by other methods cannot be used here; the raw counts are required!
...
## Good practices for reproducible analyses {.columns-2}

- Take time to fully read and understand package vignettes! 'Quick start' guides are rarely sufficiently complex to properly represent the real-world.

- If packages use (pseudo-)random routines, make sure to set *seed values* (e.g. with `set.seed()`) so that repeated code runs produce identical results.

- Use `sessionInfo()` to record the versions of `R` and the packages used.

- Make effective use of markdown document structures to layout your work.

- Use code comments to record your thoughts.

<p class="forceBreak"></p>

- Make use of `summary()`, `str()`, `head()`, and `View()` to check things are as you expect.

- After significant computations, save intermediate objects to allow easy roll-backs.

- Use a clean and consistent directory structure:

<center>
![](images/3_analysis_layout.png){width=50%}
</center>
...
Using your own data or the penguin-related data available within the homework, `./content/Homework_3.Rmd`, try out an analysis of your own. You can do whatever you like here - the point is that you practice.
